...
function actualizar() {
  ...
  cx.fillStyle = "black";
  cx.textAlign = "center";
  cx.fillText("Ordena por Color", cv.width/2, 50);

  // loop to make lines in circle
  var j;
  var k;
  var q_colors = Object.keys(color_count).length;
	var radio = 100;
	var ix = 200;
	var iy = 400;
  var f = Math.PI / n * 2;
  ...
  for (j = 0; j < q_colors ; j++) {
    var n = Object.values(color_count)[j];
    var f = Math.PI / n * 2;
    for (k = 0; k < Object.values(color_count)[j]; k++){
...
window.onload = function() {
  ...
  for (i = 0; i < quantity; i++) {
    if (amount_col < 5){ 
      item_color = colors[Math.floor(Math.random() * colors.length)];
    }
    else{
      item_color = current_colors2[Math.floor(Math.random() * current_colors2.length)];
    }
    
    var item_shape = shapes[Math.floor(Math.random() * shapes.length)];
    if (!(current_colors.includes(item_color ))){ amount_col+=1; }
